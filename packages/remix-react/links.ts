...
import type { DataRouteMatch } from "react-router-dom";
...
export async function getStylesheetPrefetchLinks(
  ...
  let links = await Promise.all(
    matches.map(async (match) => {
      let mod = await loadRouteModule(match.route, manifest, routeModules);
...
export function getNewMatchesForLinks(
  ...
  // TODO: Export util from router to share this logic
  let newMatches =
